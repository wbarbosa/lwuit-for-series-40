...
public class LocalApp {
    ...
    private static final String YAHOO_APP_ID =
        "VS2gtQrV34ElS4obpTabGJ0lxYxDjwPzrjgaj_xTo.VbdnpA24586Jul4oDCXpO3UVN7";
    private static final String NOKIA_APP_ID = "PZ6YdHu8OO_yigzWzCck";
    private static final String NOKIA_APP_TOKEN = "C8aPE3ub_GxekV5NZicAFw";
    private static final String LOCAL_BASE =
        "http://local.yahooapis.com/LocalSearchService/V2/localSearch";
    private static final String MAP_BASE = "http://m.nok.it/";
    ...
    private void setTheme(String name) {
        try {
            ...
            com.sun.lwuit.plaf.UIManager.getInstance().setThemeProps(res.getTheme(res.
    ...
    public void startApp() {
        ...
        if (!started) {
            ...
            /* try { com.sun.lwuit.util.Resources res = com.sun.lwuit.util.Resources.open("/theme.res");
    ...
    private void showMainForm() {
        ...
        mainForm.setTransitionInAnimator(CommonTransitions.createFastSlide(
            CommonTransitions.SLIDE_HORIZONTAL, false, 400));
        mainForm.setTransitionOutAnimator(CommonTransitions.createFastSlide(
        ...
        final ComboBox sortResults = new ComboBox(new String[]{"Distance", "Title", "Rating",
                "Relevance"});
        ...
        Command searchCommand = new Command("Search") {
            ...
            public void actionPerformed(ActionEvent ev) {
                showSearchResultForm(searchFor.getText(), location.getText(), street.getText(),
                    (String) sortResults.getSelectedItem());
            }
        };
    ...
    private void showSearchResultForm(String searchFor, String location, String street,
        String sortOrder) {
        ...
        final List resultList = new List(
            new LocalResultModel(searchFor, location, sortOrder, street)) {
        ...
        resultList.setRenderer(new DefaultListCellRenderer(false) {
            ...
            public Component getListCellRendererComponent(List list, Object value, int index,
                ...
                if (isSelected) {
                    ...
                    secondLine.setText(l.distance + " miles " + ("".equals(l.rating) ? "" : ", "
                        + l.rating + "*"));
                    return selected;
                }
    ...
    private void showDetails(final Form resultForm, final Object selectedItem) {
        if (selectedItem != null && selectedItem instanceof Links) {
            ...
            details.setTransitionInAnimator(CommonTransitions.createFastSlide(
                CommonTransitions.SLIDE_HORIZONTAL, false, 400));
            details.setTransitionOutAnimator(CommonTransitions.createFastSlide(
                CommonTransitions.SLIDE_HORIZONTAL, true, 400));
            details.show();
        }
    }
        ...
        if (selectedItem instanceof Links) {
            ...
            /*ConnectionRequest con = new ConnectionRequest() {
                ...
                protected void readResponse(InputStream input)
                    ...
                    /* JSONParser p = new JSONParser(); Hashtable results = (Hashtable) p.parse(new
                     * InputStreamReader(input)).get( "ResultSet"); String url = (String)
                     * results.get("Result"); img.setUrl(url); img.addRequestHeader("Accept",
                     * "image/png"); img.setDisposeOnCompletion(progress);
                    NetworkManager.getInstance().addToQueue(img); 
                    mapLabel.setIcon(Image.createImage(input));
                }
            };*/
            img.setUrl(MAP_BASE);
            img.addRequestHeader("Accept", "image/png");
            img.setDisposeOnCompletion(progress);
            img.setPriority(ConnectionRequest.PRIORITY_HIGH);
            img.setPost(false);
            img.addRequestHeader("Accept", "image/png");
            img.addArgument("app_id", NOKIA_APP_ID);
            img.addArgument("token", NOKIA_APP_TOKEN);
            img.addArgument("c", (link.latitude + "," + link.longitude));
            img.addArgument("h", Integer.toString((int) (Display.getInstance().
                getDisplayHeight() * 1.5)));
            img.addArgument("w", Integer.toString((int) (Display.getInstance().
                getDisplayWidth() * 1.5)));
            img.addArgument("z", Integer.toString(zoom));
            img.addArgument("f", "0");
            img.addArgument("nord", "");

            if (!isLWUITTheme()) {
                progress.getDialogStyle().setBorder(Border.createRoundBorder(6, 6, 0xe3ef5a));
            }
            progress.setTransitionInAnimator(CommonTransitions.createSlide(
                CommonTransitions.SLIDE_VERTICAL, true, 400));
            progress.setTransitionOutAnimator(CommonTransitions.createSlide(
                CommonTransitions.SLIDE_VERTICAL, false, 400));
            ((FlowLayout) progress.getLayout()).setValign(Component.CENTER);
            progress.addComponent(new Label("Please Wait"));
            SliderBridge b = new SliderBridge(new ConnectionRequest[]{img});
            ...
            map.setTransitionInAnimator(CommonTransitions.createFastSlide(
                CommonTransitions.SLIDE_HORIZONTAL, false, 400));
            map.setTransitionOutAnimator(CommonTransitions.createFastSlide(
                CommonTransitions.SLIDE_HORIZONTAL, true, 400));
    ...
    class LocalResultModel
        ...
        private ConnectionRequest fetchEntry(final int startOffset) {
            ConnectionRequest req = new ConnectionRequest() {
                ...
                protected void readResponse(InputStream input)
                    ...
                    if (startOffset == 1) {
                        ...
                        cache.setSize(Math.min(250, ((Double) result.get("totalResultsAvailable")).
                            intValue()));
                    }
            ...
            req.addArgument("appid", YAHOO_APP_ID);
