...
/**
 * Renders an image from lwuit on a Swing canvas including support for LWUIT animations
 *
 * @author Shai Almog
 */
public class LWUITImageRenderer extends JComponent implements MouseListener, MouseMotionListener {
    private com.sun.lwuit.Image image;
    ...
    public LWUITImageRenderer(com.sun.lwuit.Image image) {
        this.image = image;
        if(image instanceof StaticAnimation) {
            StaticAnimation anim = (StaticAnimation)image;
            buffer = new BufferedImage[anim.getFrameCount()];
            times = new int[buffer.length];
            totalAnimationTime = anim.getTotalAnimationTime();
            for(int iter = 0 ; iter < buffer.length ; iter++) {
                buffer[iter] = new BufferedImage(image.getWidth(), image.getHeight(), BufferedImage.TYPE_INT_RGB);
                buffer[iter].setRGB(0, 0, image.getWidth(), image.getHeight(), anim.getFrameRGB(iter), 0, image.getWidth());
                times[iter] = anim.getFrameTime(iter);
            }
    ...
    public static LWUITImageRenderer create(File file) throws IOException {
        ...
        LWUITImageRenderer r = new LWUITImageRenderer(com.sun.lwuit.EncodedImage.create(i));
        i.close();
        return r;
    }
