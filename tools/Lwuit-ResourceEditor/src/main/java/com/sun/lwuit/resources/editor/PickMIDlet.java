            ...
            if(jar != null) {
                try {
                    ...
                    while(en.hasMoreElements()) {
                        ...
                        if(className.endsWith(".class") && !className.startsWith("com.sun.lwuit.") && className.indexOf('$') < 0) {
                            className = className.substring(0, className.length() - 6);
                            try {
                                CustomComponent current = new CustomComponent();
                                current.setCls(cl.loadClass(className));
                                if(!current.getCls().getName().startsWith("com.sun.lwuit")) {
                                    // check that it has a default constructor
                                    com.sun.lwuit.Component cmp = (com.sun.lwuit.Component)current.getCls().newInstance();
                                    current.setClassName(current.getCls().getName());
                                    current.setLwuitBaseClass(current.getCls().getName());
        ...
        if(jar != null && midlet != null) {
            ...
            if(jarFile.exists()) {
                try {
                    ...
                    SwingImplementation.setClassLoader(cls);
                    EditableResources.setResourcesClassLoader(cls);
                    Accessor.setTheme(themeHash);
                    
                    MIDlet app = (MIDlet) cls.newInstance();
                    ...
                    while(iter.hasNext()){
                        Attributes.Name name = (Name) iter.next();
                        app.putAppProperty(name.toString(), m.getValue(name));
                    }
                    
                    MIDletAccessor.start(app);
