 ...
 * @author Shai Almog
 */
public class ImageMultiEditor extends javax.swing.JPanel {
    private EditableResources res;
    private String name;
    private static final int DPIS[] = {
            com.sun.lwuit.Display.DENSITY_VERY_LOW,
            com.sun.lwuit.Display.DENSITY_LOW,
            com.sun.lwuit.Display.DENSITY_MEDIUM,
            com.sun.lwuit.Display.DENSITY_HIGH,
            com.sun.lwuit.Display.DENSITY_VERY_HIGH,
            com.sun.lwuit.Display.DENSITY_HD
    };
    private EditableResources.MultiImage multi;
    private LWUITImageRenderer renderer;
    
    /** Creates new form ImageMultiEditor */
    public ImageMultiEditor(EditableResources res, String name) {
        initComponents();
        dpi.setRenderer(new DefaultListCellRenderer() {
            @Override
            public Component getListCellRendererComponent(JList list, Object value, int index, boolean isSelected, boolean cellHasFocus) {
                if(index > -1) {
                    com.sun.lwuit.Image i = getByDPI(multi, DPIS[index]);
                    if(i != null) {
        ...
        if(res.getResourceObject(name) != null) {
            ...
            multi.setInternalImages(new com.sun.lwuit.EncodedImage[0]);
            res.setMultiImage(name, multi);
        }
    }

    private com.sun.lwuit.Image getByDPI(EditableResources.MultiImage m, int dpi) {
    ...
    public void setImage(EditableResources.MultiImage m) {
        multi = m;
        com.sun.lwuit.Image img = getByDPI(m, DPIS[dpi.getSelectedIndex()]);
        if(img == null) {
            preview.removeAll();
            preview.revalidate();
            preview.repaint();
        ...
        if(img instanceof com.sun.lwuit.EncodedImage) {
            int s = ((com.sun.lwuit.EncodedImage)img).getImageData().length;
    ...
    public static EditableResources.MultiImage scaleMultiImage(int fromDPI, int toDPI, int scaledWidth, int scaledHeight, EditableResources.MultiImage multi) {
        try {
            ...
            com.sun.lwuit.EncodedImage[] imgs = multi.getInternalImages();
            int fromOffset = -1;
            int toOffset = -1;
            for (int iter = 0; iter < dpis.length; iter++) {
            ...
            if (toOffset == -1) {
                com.sun.lwuit.EncodedImage[] newImages = new com.sun.lwuit.EncodedImage[imgs.length + 1];
                ...
                com.sun.lwuit.EncodedImage[] newImages = new com.sun.lwuit.EncodedImage[imgs.length];
                System.arraycopy(multi.getInternalImages(), 0, newImages, 0, imgs.length);
                newImage.setDpi(dpis);
                newImage.setInternalImages(newImages);
            }
            com.sun.lwuit.Image sourceImage = newImage.getInternalImages()[fromOffset];
            ...
            com.sun.lwuit.EncodedImage encoded = com.sun.lwuit.EncodedImage.create(bytes);
    ...
    private void pickFile() {
        try {
            ...
            di.readFully(data);
            di.close();
            com.sun.lwuit.EncodedImage i = com.sun.lwuit.EncodedImage.create(data);
            int[] dpis = multi.getDpi();
            int dpi = DPIS[this.dpi.getSelectedIndex()];
            com.sun.lwuit.EncodedImage[] imgs = multi.getInternalImages();
            for(int iter = 0 ; iter < dpis.length ; iter++) {
