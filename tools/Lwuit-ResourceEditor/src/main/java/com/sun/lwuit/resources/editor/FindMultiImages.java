...
public class FindMultiImages extends javax.swing.JDialog {
    ...
    private void okActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_okActionPerformed
        updateLabelImages(veryLow, com.sun.lwuit.Display.DENSITY_VERY_LOW);
        updateLabelImages(low, com.sun.lwuit.Display.DENSITY_LOW);
        updateLabelImages(medium, com.sun.lwuit.Display.DENSITY_MEDIUM);
        updateLabelImages(high, com.sun.lwuit.Display.DENSITY_HIGH);
        updateLabelImages(veryHigh, com.sun.lwuit.Display.DENSITY_VERY_HIGH);
        updateLabelImages(hd, com.sun.lwuit.Display.DENSITY_HD);
        dispose();
            ...
            for(File f : fileList) {
                try {
                    if(ImageIO.read(f) != null) {
                        com.sun.lwuit.Image img = res.getImage(f.getName());
                        if(img == null) {
                            // create new multi-image
                            EditableResources.MultiImage newImage = new EditableResources.MultiImage();

                            newImage.setDpi(new int[] {dpi});
                            newImage.setInternalImages(new com.sun.lwuit.EncodedImage[] {com.sun.lwuit.EncodedImage.create(new FileInputStream(f))});
                            ...
                            if(o instanceof EditableResources.MultiImage) {
                                ...
                                com.sun.lwuit.EncodedImage[] images = new com.sun.lwuit.EncodedImage[existing.getDpi().length + 1];
                                System.arraycopy(existing.getInternalImages(), 0, images, 0, existing.getDpi().length);
                                images[existing.getDpi().length] = com.sun.lwuit.EncodedImage.create(new FileInputStream(f));
