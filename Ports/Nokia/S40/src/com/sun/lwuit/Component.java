...
public class Component implements Animation, StyleListener {
     ...
     */
    protected Component() {
        selectText = (Display.getInstance().getDeviceType() == 
                        Display.NON_TOUCH_DEVICE) ? selectText : "";
    ...
    void paintGlassImpl(Graphics g) {
        ...
        if(tensileHighlightIntensity > 0) {
            ...
            if(scrollY >= i - 1) {
                UIManager.getInstance().getLookAndFeel().paintTensileHighlight(g, false , tensileHighlightIntensity);
            } else {
                if(scrollY < 1) {
                    UIManager.getInstance().getLookAndFeel().paintTensileHighlight(g, true, tensileHighlightIntensity);
                } else {
