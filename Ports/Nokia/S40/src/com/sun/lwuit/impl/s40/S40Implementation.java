...
public class S40Implementation extends LWUITImplementation {
    ...
    /***********************************************************************
     * Everything below here is from LWUIT's MIDP GameCanvasImplementation *
     * This is here because we need to override a few things in how        *
     * Commands are handled, and unfortunately LWUIT hides that in the     *
     * inner class C of GameCanvasImplementation.                          *
     ***********************************************************************/
    ...
    private class C extends GameCanvas implements CommandListener, Runnable {
        ...
        private Vector currentCommands;
        ...
        public void commandAction(Command c, Displayable d) {
            if (d == currentTextBox) {
                ...
                MIDPCommandWrapper temp;
                for (int i = 0; i < l; ++i) {
                    temp = (MIDPCommandWrapper) currentCommands.elementAt(i);
                    if (c == temp.getCommand()) {
                        final com.sun.lwuit.Command cmd = temp.getLWUITCommand();
        ...
        protected void keyPressed(final int keyCode) {
            S40Implementation.this.keyPressed(keyCode);
        }
        ...
        protected void hideNotify() {
            S40Implementation.this.hideNotify();
        }
...
// Check Third Soft Button is supported        
    ...
    public void editString(Component cmp, int maxSize, int constraint, String text, int keyCode) {
        ...
        ((S40Implementation.C)canvas).setDone(false);
        Display.getInstance().invokeAndBlock(((S40Implementation.C)canvas));
    }
    ...
    public void flushGraphics(int x, int y, int width, int height) {
        ...
        com.sun.lwuit.Form current = getCurrentForm();
    ...
    public VideoComponent createVideoPeer(String url) throws IOException {
        try {
            ...
            p.realize();
            return new S40Implementation.MIDPVideoComponent(p);
    ...
    public void addNativeCommand(com.sun.lwuit.Command c) {
        ((S40Implementation.C)canvas).addCommand(c);
    }
