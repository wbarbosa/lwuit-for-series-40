...
public class XMLParser {
   ...
   public XMLParser() {
    ...
    protected Element parseCommentOrXMLDeclaration(Reader is,String endTag) throws IOException {
        String text="";
        boolean endTagFound = true;
        while (!text.endsWith(endTag)) {
            int c = is.read();
            if (c == -1) { // EOF
                endTagFound = false;
                break;
        ...
        if (endTagFound) // Remove endTag from text
        ...
        if (!endTagFound)
                notifyError(ParserCallback.ERROR_NO_CLOSE_TAG, elementName, null,
                        null, "Malformed XML - no appropriate closing tag for " + elementName);

        if (!endTag.equals("-->") && !endTag.equals(">")) { //CDATA
            if(eventParser) {
                textElement(text);
                return null;
            }
