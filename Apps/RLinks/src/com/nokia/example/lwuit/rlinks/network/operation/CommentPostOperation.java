...
package com.nokia.example.lwuit.rlinks.network.operation;

import com.nokia.example.lwuit.rlinks.network.HttpOperation;
import com.nokia.example.lwuit.rlinks.util.UrlEncoder;
...
public class CommentPostOperation
    ...
    public String getRequestBody() {
        return "thing_id=" + UrlEncoder.encode(thingName)
    ...
    public void responseReceived(byte[] response) {
        ...
        if (response == null || response.length == 0) {
            System.out.println("Error response null or empty");
    ...
    private void parseResponse(String responseJson) {
        if (responseJson.indexOf(".error.USER_REQUIRED") >= 0) {
            System.out.println("Error USER_REQUIRED");
            listener.commentingFailed(thingName, text);
        }
        else if (responseJson.indexOf(".error.RATELIMIT") >= 0) {
            System.out.println("Error RATELIMIT");
            listener.commentingFailed(thingName, text);
        }
        else {
